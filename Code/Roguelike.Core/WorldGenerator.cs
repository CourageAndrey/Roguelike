...
namespace Roguelike.Core
{
	public static class WorldGenerator
	{
		...
		public static void CreateVillage(this Region region, Balance balance, Random seed, int x1, int x2, int y1, int y2, int z)
		{
			...
			{
				var husband = new Npc(true, Time.FromYears(balance, 50), new Properties(), new Inventory(), "John Smith " + i);
				husband.placeIntoFreeCell(region, seed, x1, x2, y1, y2, z);

				var wife = new Npc(false, Time.FromYears(balance, 50), new Properties(), new Inventory(), "Mary Poppins " + i);
				wife.placeIntoFreeCell(region, seed, x1, x2, y1, y2, z);

				var pet = new Animal(false, Time.FromYears(balance, 5), new Properties(), new Inventory()) { Owner = husband };
